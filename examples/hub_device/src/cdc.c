...
static const uint32_t interval = 2000000; // 2 seconds
static const char msg[] = "Hello from CDC!\n";
...
usbd_handle_t* cdc_init(usbd_hw_type_t hw_type) {
    ...
    return usbd_init(hw_type, &cdc_driver, CDC_DESC_DEVICE.bMaxPacketSize0);
}

void cdc_task(usbd_handle_t* handle) {
    if (((time_us_32() - start) > interval) && 
        usbd_ep_ready(handle, CDC_DATA_EPADDR_IN)) {
        usbd_ep_write(handle, CDC_DATA_EPADDR_IN, msg, sizeof(msg));
        start = time_us_32();
    }
}